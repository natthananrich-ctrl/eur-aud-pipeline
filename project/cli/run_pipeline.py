...
run_pipeline.py
---------------
Pipeline พร้อม Error Handling + AuditLogger + AutoRetrain
"""
...
def run_pipeline(env="dev"):
    ...
    try:
        ...
        try:
            ...
            df_features = pd.DataFrame()  # fallback
        ...
        # 6. Retrain if drift detected
        try:
            if drift_result and drift_result.get("overall_drift", False):
                retrainer = AutoRetrain(env=env)
                retrain_result = retrainer.run(df_features, drift_flag=True)
                logger.log_event("models", "auto_retrain", "SUCCESS", f"retrain_result={retrain_result}")
            else:
                logger.log_event("models", "auto_retrain", "SKIPPED", "No drift detected")
        except Exception as e:
            logger.log_event("models", "auto_retrain", "FAIL", str(e))
